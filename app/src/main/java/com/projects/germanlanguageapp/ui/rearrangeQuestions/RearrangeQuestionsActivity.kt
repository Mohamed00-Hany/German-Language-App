...
class RearrangeQuestionsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.e("hany",levelId.toString())
        Log.e("hany",lessonId.toString())
    ...
    private fun checkAnswer() {
        val correctAnswer = viewModel.questions.value!![currentQuestionsIndex]?.rearrangeNameAnswer?.toLowerCase()?.replace("\\s".toRegex(), "")
        val userAnswer = "\"${binding.answerText.text.toString().toLowerCase(Locale.ROOT)}\"".replace("\\s".toRegex(), "")
